...
from repositories import fetch_historical_inventory_data


def load_local_store_data(conn: pyodbc.Connection, store: str | None, date_range: list[datetime] | None = None) -> dict | None:
    """Load store data for use in report headers.

    Args:
        conn: Database connection object
        store: Store number inputted by user or None if retrieving aggregate date range stats
    ...
    try:
        ...
        if store:
            inventory_row = fetch_historical_inventory_data(conn, store)
            ...
            store_data["inventory_datetime"] = inventory_row[0].strftime("%m/%d/%Y %I:%M:%S %p") or ""
        ...
        QtWidgets.QMessageBox.warning(
            None,
            "Database Error",
            f"A database operation failed while loading local or aggregate date range store data.\n\nDetails:\n{str(e)}"
        )
        raise
