...
class EmpHoursInputWindow(QtWidgets.QMainWindow):
    ...
    def print_report(self) -> None:
        ...
        save_local_data(conn=conn, store_data=self.store_data, emp_data=self.emp_data, zone_data=self.zone_data)

        if conn:
            conn.close()

        generate_accuracy_report(store_data=self.store_data, emp_data=self.emp_data, zone_data=self.zone_data, season=False)
